...
import { Music, MessageSquare, Play, Pause, SkipForward, SkipBack, Send, Minimize2, Settings, AlertTriangle } from 'lucide-react';
...
export const GlobalToolbox: React.FC = () => {
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
      ...
      if (!chatSessionRef.current) {
          ...
          setMessages(prev => [...prev, { role: 'model', text: "Sorry, I can't think right now (API Key issue)." }]);
          setInput('');
          return;
      }
      ...
      {isOpen && (
        <div className="mb-4 bg-white border-4 border-black rounded-xl shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] w-80 md:w-96 overflow-hidden animate-in slide-in-from-bottom-10 fade-in duration-300">
           ...
           <div className="flex border-b-4 border-black bg-gray-100">
              ...
              <button onClick={() => setIsOpen(false)} className="p-3 bg-toon-red text-white border-l-4 border-black hover:bg-red-600">
                <Minimize2 size={18} />
              </button>
           </div>
           ...
           <div className="h-80 bg-white relative">
              ...
              {activeTab === 'music' && (
                  <div className="h-full flex flex-col p-6 items-center justify-center bg-gray-50 text-gray-900">
                      <div className={`w-32 h-32 bg-gray-900 rounded-full border-4 border-black flex items-center justify-center mb-6 shadow-toon ${isPlaying ? 'animate-[spin_4s_linear_infinite]' : ''}`}>
                          <Music size={48} className="text-toon-yellow" />
                      </div>
                      
                      <div className="text-center mb-6">
                          <h3 className="font-black text-xl truncate w-64">{PLAYLIST[currentTrackIndex].title}</h3>
                          <p className="font-bold text-gray-500">{PLAYLIST[currentTrackIndex].artist}</p>
                      </div>

                      <div className="flex items-center gap-4">
                          <button onClick={handlePrevTrack} className="p-2 border-2 border-black rounded-lg hover:bg-gray-200 bg-white shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] active:translate-y-[2px] active:shadow-none transition-all">
                              <SkipBack size={24} className="text-gray-900" />
                          </button>
                          <button onClick={togglePlay} className="p-4 border-2 border-black rounded-full bg-toon-yellow hover:bg-yellow-400 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] active:translate-y-[2px] active:shadow-none transition-all">
                              {isPlaying ? <Pause size={32} className="text-toon-ink" /> : <Play size={32} className="pl-1 text-toon-ink" />}
                          </button>
                          <button onClick={handleNextTrack} className="p-2 border-2 border-black rounded-lg hover:bg-gray-200 bg-white shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] active:translate-y-[2px] active:shadow-none transition-all">
              ...
              {activeTab === 'chat' && (
                  <div className="h-full flex flex-col bg-white">
                      <div className="flex-1 overflow-y-auto p-4 space-y-3">
                          ...
                          {messages.map((msg, idx) => (
                              <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                                  <div className={`max-w-[85%] p-3 rounded-xl border-2 border-black font-medium text-sm ${msg.role === 'user' ? 'bg-toon-yellow text-toon-ink rounded-br-none' : 'bg-gray-100 text-gray-900 rounded-bl-none'}`}>
                                      {msg.text}
                                  </div>
                              </div>
                          ))}
                      ...
                      <form onSubmit={handleSendMessage} className="p-3 border-t-2 border-black bg-gray-50 flex gap-2">
                          <input 
                              value={input}
